...
class CartTest {
    ...
    public void fineCartAndMemberTest() {
        ...
        cart.setMember(member); // 생성한 회원 객체를 cart 객체에 연결(장바구니와 회원간의 연관관계)
        cartRepository.save(cart); //연관관계가 설정된 cart 객체를 장바구니 저장소에 저장

        em.flush(); //변경된 내용을 데이터 베이스에 즉시 반영
        em.clear(); //영속성 컨텍스트 초기화
        Cart savedCart = cartRepository.findById(cart.getId())
                .orElseThrow(EntityNotFoundException::new);
        //저장된 cart 객체를 데이터 베이스에서 조회
        assertEquals(savedCart.getMember().getId(), member.getId());
        //조회한 cart 객체의 회원 id와 원래 회원 객체의 id를 비교 테스트
    }
}
