...
public class OrderService {
    ...
    public Long order(OrderDto orderDto, String email) {
        ...
        //주문을 생성하는 회워 memberRepository 에서 이메일을 이용해 조회

        List<OrderItem> orderItemList = new ArrayList<>();//주문항목 리스트를 생성
        OrderItem orderItem = OrderItem.createOrderItem(item, orderDto.getCount());
        //주문 항목을 생성 OrderItem 클래스의 createOrderItem 메서드를 호출해서 주문 항목을 생성
        // 이때 필요한 정보 아이템과 주문 수량
        orderItemList.add(orderItem); // 생성한 주문 항목을 주문 항목 리스트에 추가
        Order order = Order.createOrder(member, orderItemList); // 주문을 생성합니다.
        //Order 클래스의 createOrder 메서드를 호출하여 주문을 생성하며,
        //이때 회원 정보와 주문 항목 리스크가 제공됩니다.

        orderRepository.save(order);
        //생성한 주문을 데이커 베이스에 저장합니다.
    ...
    public Page<OrderHistDto> getOrderList(String email, Pageable pageable) {
        ...
        //주문 레포지토리에서 입력 받은 메일주소와 페이징 정보를 기반으로 주문 목록 조회
